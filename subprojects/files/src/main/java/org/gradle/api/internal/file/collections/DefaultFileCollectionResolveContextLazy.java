...
public class DefaultFileCollectionResolveContextLazy implements ResolvableFileCollectionResolveContext {
    ...
    public DefaultFileCollectionResolveContextLazy(FileResolver fileResolver) {
        this(fileResolver, new FileCollectionConverter(fileResolver.getPatternSetFactory()), new FileTreeConverter(fileResolver.getPatternSetFactory()), new ArrayDeque<>());
    }

    public DefaultFileCollectionResolveContextLazy(PathToFileResolver fileResolver, Converter<? extends FileCollectionInternal> fileCollectionConverter, Converter<? extends FileTreeInternal> fileTreeConverter, Queue<Object> queue) {
    ...
    protected DefaultFileCollectionResolveContextLazy newContext(PathToFileResolver fileResolver) {
        return new DefaultFileCollectionResolveContextLazy(fileResolver, fileCollectionConverter, fileTreeConverter, new ArrayDeque<>());
    }

    @Override
    public final DefaultFileCollectionResolveContextLazy newContext() {
    ...
    private <T> Stream<T> doResolve(Converter<T> converter) {
        Iterator<T> queueConsumer = new Iterator<T>() {
            private Iterator<T> next = Collections.emptyIterator();

            @Override
            public boolean hasNext() {
                return next.hasNext() || !queue.isEmpty();
            }
    ...
    private <T> Stream<T> nextBatch(Converter<T> converter, Object element) {
        if (element instanceof DefaultFileCollectionResolveContextLazy) {
    ...
    private <T> Stream<T> resolveNested(Converter<T> converter, FileCollectionContainer element) {
        DefaultFileCollectionResolveContextLazy context = newContext();
        element.visitContents(context);
        return resolveElements(converter, context.queue);
    }

    public interface Converter<T> {
        Stream<T> convertInto(Object element, PathToFileResolver resolver);
    }
    ...
    public static class FileCollectionConverter implements Converter<FileCollectionInternal> {
        ...
        @Override
        public Stream<FileCollectionInternal> convertInto(Object element, PathToFileResolver fileResolver) {
            if (element instanceof DefaultFileCollectionResolveContextLazy) {
                return ((DefaultFileCollectionResolveContextLazy) element).resolveAsFileCollections().stream();
    ...
    public static class FileTreeConverter implements Converter<FileTreeInternal> {
        ...
        public Stream<FileTreeInternal> convertInto(Object element, PathToFileResolver fileResolver) {
            if (element instanceof DefaultFileCollectionResolveContextLazy) {
                return ((DefaultFileCollectionResolveContextLazy) element).resolveAsFileTrees().stream();
    ...
    public static class MinimalFileCollectionConverter implements Converter<MinimalFileCollection> {
        ...
        public Stream<MinimalFileCollection> convertInto(Object element, PathToFileResolver resolver) {
            if (element instanceof DefaultFileCollectionResolveContextLazy) {
                return ((DefaultFileCollectionResolveContextLazy) element).resolveAsMinimalFileCollections().stream();
            } else if (element instanceof MinimalFileCollection) {
