...
public class DefaultArtifactHandler implements ArtifactHandler, MethodMixIn {
    ...
    private PublishArtifact pushArtifact(Configuration configuration, Object notation, Action<? super ConfigurablePublishArtifact> configureAction) {
        warnIfConfigurationIsDeprecated((ConfigurationInternal) configuration);
    ...
    private void warnIfConfigurationIsDeprecated(ConfigurationInternal configuration) {
        List<String> alternatives = configuration.getDeclarationAlternatives();
        if (alternatives != null) {
            DeprecationLogger.nagUserOfReplacedConfiguration(configuration.getName(), DeprecationLogger.ConfigurationDeprecationType.REMOVAL, alternatives);
        }
    }
