...
public class GroovyCompile extends AbstractCompile {
    ...
    private final File sourceToClassesMapFile;
    ...
    public GroovyCompile() {
        ...
        this.sourceToClassesMapFile = new File(getTemporaryDir(), "source-classes-mapping.txt");
    ...
    protected void compile(InputChanges inputChanges) {
        ...
        if (isIncremental(inputChanges)) {
            Multimap<File, String> oldMappings = readSourceClassesMappingFile(sourceToClassesMapFile);
    ...
    public static Multimap<File, String> readSourceClassesMappingFile(File mappingFile) {
        ...
        Multimap<File, String> sourceToClassesMap = MultimapBuilder.ListMultimapBuilder
            .hashKeys()
            .arrayListValues()
            .build();
        ...
        try {
            Files.asCharSource(mappingFile, Charsets.UTF_8).readLines(new LineProcessor<Void>() {
                ...
                public boolean processLine(String line) throws IOException {
                    if (line.startsWith(" ")) {
                        sourceToClassesMap.put(currentFile, line.substring(1));
                    } else {
    ...
    private void updateSourceClassesMappingFile(InputChanges inputChanges, Multimap<File, String> oldMappings) {
        Multimap<File, String> mappingsDuringIncrementalCompilation = readSourceClassesMappingFile(sourceToClassesMapFile);
        ...
        for (FileChange fileChange : inputChanges.getFileChanges(getStableSources())) {
            ...
            Collection<String> classes = mappingsDuringIncrementalCompilation.get(changedFile);
            if (!classes.isEmpty()) {
                oldMappings.putAll(changedFile, classes);
            }
        }
    }

    protected boolean isIncremental(InputChanges inputChanges) {
        return getOptions().isIncremental() && inputChanges != null && inputChanges.isIncremental() && sourceToClassesMapFile.isFile();
    }
    ...
    private Compiler<GroovyJavaJointCompileSpec> getCompiler(GroovyJavaJointCompileSpec spec) {
        return getCompiler(spec, null, null);
    }

    private Compiler<GroovyJavaJointCompileSpec> getCompiler(GroovyJavaJointCompileSpec spec, InputChanges inputChanges, Multimap<File, String> sourceToClassesMap) {
        if (compiler == null) {
            ...
            if (isIncremental(inputChanges)) {
                ...
                return factory.makeIncremental(cleaningGroovyCompiler, getPath(), new IncrementalInputsAdapter(inputChanges, getStableSources()), getStableSources().getAsFileTree(), ".groovy", new Function<CompilationSourceDirs, SourceToNameConverter>() {
                    ...
                    public SourceToNameConverter apply(CompilationSourceDirs sourceDirs) {
                        return new GroovySourceToNameConverter(sourceDirs, sourceToClassesMap);
                    }
                });
            } else {
    ...
    private DefaultGroovyJavaJointCompileSpec createSpec() {
        ...
        if (getOptions().isIncremental()) {
            spec.setCompilationMappingFile(sourceToClassesMapFile);
            sourceToClassesMapFile.delete();
        }
